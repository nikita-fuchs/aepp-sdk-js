...
function enterState(channel: Channel, nextState: ChannelFsm): void {
  ...
  if (channel._options.debug === true) {
    console.log(`Channel #${channel._options.debugId} enter state`, nextState.handler.name);
  }
...
export function changeStatus(channel: Channel, newStatus: ChannelStatus, debug?: unknown): void {
  if (channel._options.debug === true) {
    console.log(`Channel #${channel._options.debugId}`, newStatus, `(prev. ${channel._status})`, debug ?? '');
  }
...
function send(channel: Channel, message: ChannelMessage): void {
  if (channel._options.debug === true) {
    console.log(`Channel #${channel._options.debugId} send message`, message.method, message.params);
  }
...
function onMessage(channel: Channel, data: string): void {
  ...
  if (channel._options.debug === true) {
    console.log(`Channel #${channel._options.debugId} received message`, message.method, message.params);
  }
  if (message.id != null) {
...
export async function initialize(
  ...
  channelCounter += 1;
  channel._options.debugId = channelCounter;
